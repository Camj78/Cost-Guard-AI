...
export function useShaderBackground() {
  ...
  useEffect(() => {
    // StrictMode guard — prevent double RAF loops
    ...
    // WebGL2 availability guard
    const gl = canvas.getContext("webgl2");
    if (!gl) { isRunning.current = false; return; }
    ...
    const renderer = new WebGLRenderer(canvas, dpr, gl);
    ...
    const resize = () => {
      ...
      canvas.height = canvas.parentElement
        ? canvas.parentElement.clientHeight * d
...
// ─── Full Hero Layout Component (for standalone use) ─────────────────────────
...
const Hero: FC<HeroProps> = ({
  ...
  return (
    <div className={cn("relative w-full overflow-hidden bg-background", className)}>
      {/* Injected keyframes — React 19 hoists <style> to <head> with deduplication */}
      {/* eslint-disable-next-line @typescript-eslint/ban-ts-comment */}
      {/* @ts-ignore — React 19 extended style props (href + precedence) */}
      <style href="shader-hero-animations" precedence="default">{`
        @keyframes shaderHeroFadeDown {
          from { opacity: 0; transform: translateY(-20px); }
          to   { opacity: 1; transform: translateY(0); }
        }
        @keyframes shaderHeroFadeUp {
          from { opacity: 0; transform: translateY(30px); }
          to   { opacity: 1; transform: translateY(0); }
        }
        .shader-hero-fade-down { animation: shaderHeroFadeDown 0.8s ease-out both; }
        .shader-hero-fade-up   { animation: shaderHeroFadeUp 0.8s ease-out both; opacity: 0; }
              ...
              {trustBadge.icons && (
                <div className="flex">
                  {trustBadge.icons.map((icon, index) => (
                    <span
                      key={index}
                      className={ICON_COLORS[index] ?? "text-primary"}
                    >
                      {icon}
                    </span>
                  ))}
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center mt-10 shader-hero-fade-up shader-hero-delay-800">
              {buttons.primary && (
                <button
                  ...
                  className="px-8 py-4 bg-primary text-primary-foreground hover:bg-primary/90 rounded-full font-semibold text-lg transition-colors duration-200"
                >
