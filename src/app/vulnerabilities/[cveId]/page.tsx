...
const SCORE_IMPACT: Record<string, string> = {
  critical: "CSS decreases by up to 10 points",
  high:     "CSS decreases by up to 7 points",
  medium:   "CSS decreases by up to 3 points",
};
...
export async function generateMetadata({
  ...
  return {
    title: `${cveId} | Prompt Vulnerability | CostGuardAI`,
    description: `Structural prompt vulnerability ${cveId} — severity, incident statistics, and mitigation guidance.`,
  };
}
      ...
      <main className="flex-1 px-4 sm:px-6 py-12">
        <div className="mx-auto max-w-lg space-y-4">
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="space-y-1">
                  ...
                  <p className="text-xs text-muted-foreground">
                    {RISK_TYPE_LABEL[vuln.risk_type] ?? vuln.risk_type}
                  </p>
                </div>
                <span className={`text-sm font-semibold uppercase shrink-0 ${SEVERITY_COLOR[vuln.severity] ?? "text-muted-foreground"}`}>
                  {vuln.severity}
                </span>
              </div>
            </CardContent>
          ...
          <Card className="glass-card shadow-none">
            ...
            <CardContent className="pb-4">
              <p className="text-xs text-muted-foreground leading-relaxed">
                {vuln.mitigation || "No specific mitigation guidance recorded. Apply general structural hardening: add output format constraints, section delimiters, and explicit refusal boundaries."}
              </p>
            </CardContent>
          </Card>
          ...
          <Card className="glass-card shadow-none">
            ...
            <CardContent className="pb-4 space-y-2">
              <div className="flex items-center justify-between">
                ...
                <span className={`text-xs font-semibold ${SEVERITY_COLOR[vuln.severity] ?? "text-muted-foreground"}`}>
                  {vuln.severity}
                </span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-xs text-muted-foreground">CSS Impact</span>
                <span className="font-mono tabular-nums text-xs text-muted-foreground text-right">
                  {SCORE_IMPACT[vuln.severity] ?? "—"}
                </span>
              </div>
              <p className="text-[11px] text-muted-foreground/50 pt-1 leading-relaxed">
                Impact is bounded. No single vulnerability match can push a prompt below the
                Unsafe band boundary on its own.
              </p>
            </CardContent>
          </Card>
            ...
            <CardContent className="pb-4 space-y-2">
              <div className="flex items-center justify-between">
                ...
                <span className="font-mono tabular-nums text-xs text-right">
                  {vuln.incident_count.toLocaleString()}
                </span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-xs text-muted-foreground">First seen</span>
                <span className="font-mono tabular-nums text-xs text-right">
                  {formatDate(vuln.first_seen)}
                </span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-xs text-muted-foreground">Last seen</span>
                <span className="font-mono tabular-nums text-xs text-right">
                  {formatDate(vuln.last_seen)}
                </span>
              </div>
              <div className="flex items-center justify-between">
          ...
          <Card className="glass-card shadow-none">
            <CardContent className="pt-4 pb-4">
              <p className="text-[11px] text-muted-foreground/50 leading-relaxed">
                This vulnerability reflects a structural pattern. No raw prompt text, user
